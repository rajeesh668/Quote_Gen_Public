...
# ---------------------
conversion_rates = {
    "SAR": 3.7575,  # Default value; overridden later for SAR if needed.
    "AED": 3.67,
    "QAR": 3.722,
    "OMR": 0.3846,
    "KWD": 0.32,
...
# ---------------------
# Modified Add Line Item Function (with multiline SKU processing)
# ---------------------
def add_line_item(category, model_selected, transaction_type, term, quantity, partner_type, manual_sku,
    ...
    # Check if manual_sku contains multiple lines.
    ...
    if sku_list and len(sku_list) > 1:
        for sku_input in sku_list:
            ...
            if prod_type.strip().lower() in ["hardware", "appliance", "proservices"]:
                adj_term = "-"
            # Retrieve the SKU's transaction type and classification from sku_df (case-insensitive)
            ...
            # Discount logic based on per-SKU transaction type (trans_type) and line total.
...
def recalc_boQ(boq_df):
    ...
    for idx, row in boq_df.iterrows():
        ...
        row["Disc. Price (USD)"] = f"{final_price}"
        new_rows.append(row)
    new_df = pd.DataFrame(new_rows)
    new_df = new_df[["SKU", "Description", "Term", "Quantity", "Original Price (USD)",
                     "Reseller Price (USD)", "Disc. Price (USD)",
                     "Override Discount (%)", "Additional Discount (%)",
                     "Classification", "Product Type", "Status"]]
    return new_df
...
def generate_final_quote_wrapper(shipping_cost, boq_table, currency, vat):
    # Recalculate the BOQ table first
    updated_boq = recalc_boQ(boq_table)
    # Then generate the final quote based on the updated BOQ
    final_quote = generate_final_quote(shipping_cost, updated_boq, currency, vat)
    # Return both updated BOQ and final quote (so the BOQ table is updated in the UI as well)
...
def generate_final_quote(shipping_cost, boq_data, currency, vat):
    ...
    conv_rate = conversion_rates.get(currency, 3.7575)
    if currency == "SAR":
        conv_rate = 3.7575
    df_quote = boq_data.copy()
    df_quote[f"Unit Price ({currency})"] = df_quote["Disc. Price (USD)"].astype(float).apply(lambda x: custom_round(x * conv_rate))
    ...
    total_shipping = custom_round(total_original_usd * (float(shipping_cost) / 100.0) * conv_rate)
    total_vat = (float(vat) / 100.0) * (total_subtotal + total_shipping)
    grand_total = total_subtotal + total_shipping + total_vat
    df_final_quote = df_quote[["SKU", "Description", "Term", "Quantity", f"Unit Price ({currency})", f"Total Price ({currency})"]]
    df_summary = pd.DataFrame([
        ["", "Shipping Cost", "", "", "", f"{total_shipping}"],
...
""")
    ...
    # New: Wireless License Toggle (no dropdown now)
    with gr.Row():
        use_wireless_license = gr.Checkbox(label="Enable Wireless License", value=False, visible=False)
    
    # New: Switch Support License Toggle (visible only when category is "Switches")
    with gr.Row():
    ...
    with gr.Row():
        ...
        deal_registered = gr.Checkbox(label="Deal Registration", value=True, visible=True)
        incumbency = gr.Checkbox(label="Incumbency", value=True, visible=True)
    
    # Fifth row: Always show both checkboxes now.
    ...
    # NEW: gr.State to store user's BOQ data
    ...
    # Final Quote section with a green bold heading.
    ...
    add_button.click(
        add_line_item,
        inputs=[category, model_selected, transaction_type, term, quantity, partner_type, manual_sku,
    ...
    # --- Change here: Use generate_final_quote_wrapper instead of generate_final_quote so that recalc happens first ---
